...
function legacy() {
  const { crew: _crew, city: _city, ...state } = rebirthState(37, 48);
...
describe('v9 territory schema and sequential migration', () => {
  it('adds only the Waterfront baseline to a rich v8 save, preserving every prior field exactly', () => {
    ...
    const result = parseSave(raw); expect(CURRENT_SAVE_VERSION).toBe(11);
    expect(result).toEqual({ ok: true, envelope: { ...old, version: 11,
      state: { ...old.state, crew: { recruitedIds: [], assignments: { operations: null, logistics: null } }, city: { heat: 0, heatDecayElapsedMs: 0, ownedTerritoryIds: [W.id] } } } });
    if (!result.ok) throw Error('fixture');
    const { crew: _crew, city, ...previous } = result.envelope.state;
    expect(previous).toEqual(old.state); expect(city.ownedTerritoryIds).not.toContain(N.id);
    expect(result.envelope.savedAt).toBe(old.savedAt); expect(JSON.stringify(old)).toBe(raw);
  ...
  it.each([1, 2, 3, 4, 5, 6, 7, 8])('CE1 migrates historical v%i without charging or granting Neon Mile', version => {
    ...
    expect(result).toMatchObject({ ok: true, envelope: { version: 11, savedAt: old.savedAt,
  ...
  it.each([{ ids: [W.id] }, { ids: [W.id, N.id] }])('roundtrips authoritative city %# and all previous state through CE1', ({ ids }) => {
    const state = { ...legacy().state, crew: createInitialGameState().crew, city: { heat: 0, heatDecayElapsedMs: 0, ownedTerritoryIds: ids } };
    ...
    expect(validateSaveCode(exported.code)).toEqual({ ok: true, envelope: { format: 'crime-empire-save', version: 11, savedAt: 42, state } });
  ...
  it.each([undefined, null, [], 'city', {}, { ownedTerritoryIds: null }, { ownedTerritoryIds: 'territory:waterfront' },
    ...
    expect(parseSave(JSON.stringify({ format: 'crime-empire-save', version: 11, savedAt: 1, state }))).toEqual({ ok: false, error: 'invalid-state' });
  });
  ...
  it('rejects smuggled city in v8 and unsupported future versions', () => {
    ...
    expect(parseSave(JSON.stringify({ ...old, version: 12 }))).toEqual({ ok: false, error: 'unsupported-version' });
  });
  it('migration retains savedAt for one normal offline catch-up without granting territory', () => {
    ...
    const state = { ...old.state, crew: createInitialGameState().crew, city: createInitialGameState().city };
    const expected = simulateGameElapsed(state, 25000).state;
    expect(saves.bootstrap()).toMatchObject({ kind: 'loaded', state: expected });
    expect(parseSave(raw)).toMatchObject({ ok: true, envelope: { version: 11, savedAt: now, state: expected } });
