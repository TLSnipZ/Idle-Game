...
describe('v9 territory schema and sequential migration', () => {
  it('adds only the Waterfront baseline to a rich v8 save, preserving every prior field exactly', () => {
    ...
    const result = parseSave(raw); expect(CURRENT_SAVE_VERSION).toBe(21);
    expect(result).toEqual({ ok: true, envelope: { ...old, version: 21,
  ...
  it.each([1, 2, 3, 4, 5, 6, 7, 8])('CE1 migrates historical v%i without charging or granting Neon Mile', version => {
    ...
    expect(result).toMatchObject({ ok: true, envelope: { version: 21, savedAt: old.savedAt,
  ...
  it.each([{ ids: [W.id] }, { ids: [W.id, N.id] }])('roundtrips authoritative city %# and all previous state through CE1', ({ ids }) => {
    ...
    expect(validateSaveCode(exported.code)).toEqual({ ok: true, envelope: { format: 'crime-empire-save', version: 21, savedAt: 42, state } });
  ...
  it.each([undefined, null, [], 'city', {}, { ownedTerritoryIds: null }, { ownedTerritoryIds: 'territory:waterfront' },
    ...
    expect(parseSave(stringifySaveFixture({ format: 'crime-empire-save', version: 21, savedAt: 1, state }))).toEqual({ ok: false, error: 'invalid-state' });
  });
  ...
  it('migration retains savedAt for one normal offline catch-up without granting territory', () => {
    ...
    expect(parseSave(raw)).toMatchObject({ ok: true, envelope: { version: 21, savedAt: now, state: expected } });
