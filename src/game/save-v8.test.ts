...
describe('shared v8 skill schema', () => {
  it('migrates realistic v7 by adding only empty skills; no EP spending, reset or timestamp change', () => {
    ...
    expect(result).toEqual({ ok: true, envelope: { ...old, version: 18, state: { ...old.state, events: { opportunityElapsedMs: 0, pendingEventId: null }, crew: { recruitedIds: [], assignments: { operations: null, logistics: null } }, city: { heat: 0, heatDecayElapsedMs: 0, ownedTerritoryIds: ['territory:waterfront'] },
  ...
  it('roundtrips all current fields, all max ranks and unspent EP through CE1', () => {
    ...
    expect(parseSave(serialized.serialized)).toEqual({ ok: true, envelope: { format: 'crime-empire-save', version: 18, savedAt: 42, state } });
  ...
  it.each([null, [], 'skills', { 'skill:unknown': 1 }, { [ROOT]: 0 }, { [ROOT]: -1 }, { [ROOT]: .5 },
    ...
    const old = legacy();
    expect(parseSave(stringifySaveFixture({ ...old, version: 18, state: { ...old.state, events: { opportunityElapsedMs: 0, pendingEventId: null }, crew: { recruitedIds: [], assignments: { operations: null, logistics: null } }, city: { heat: 0, heatDecayElapsedMs: 0, ownedTerritoryIds: ['territory:waterfront'] },
  ...
  it('requires current skills but rejects smuggled skills in a legacy envelope and future versions', () => {
    const old = legacy();
    expect(parseSave(stringifySaveFixture({ ...old, version: 18, state: { ...old.state, permanentProgression: { ...old.state.permanentProgression, statistics: createInitialStatistics(4), unlockedAchievementIds: [] }, events: { opportunityElapsedMs: 0, pendingEventId: null }, crew: { recruitedIds: [], assignments: { operations: null, logistics: null } }, city: { heat: 0, heatDecayElapsedMs: 0, ownedTerritoryIds: ['territory:waterfront'] } } }))).toEqual({ ok: false, error: 'invalid-state' });
