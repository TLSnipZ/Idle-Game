...
describe('Rebirth eligibility and rewards', () => {
  ...
  it.each([[20,25,4],[37,48,7],[100,100,20]])('preview and award agree: %i / %i => %i EP', (player,business,reward) => {
    ...
    expect(performRebirth(state)).toMatchObject({ok:true,reward,state:{permanentProgression:{ statistics: createInitialStatistics(1), unlockedAchievementIds: ['achievement:first-steps', 'achievement:dockside-operator', 'achievement:first-rebirth'],skills: {}, empirePoints:reward,rebirthCount:1}}});
  });
...
describe('explicit reset and retention', () => {
  it('resets every temporary field, retains garage and accumulates permanent counters immutably', () => {
    const state={...rebirthState(),permanentProgression:{ statistics: createInitialStatistics(3), unlockedAchievementIds: [],skills: {}, empirePoints:12,rebirthCount:3}};
  ...
  it('supports repeat Rebirths: 4 then 7 EP, count 2, same collection', () => {
    ...
    expect(first.permanentProgression).toEqual({ statistics: createInitialStatistics(1), unlockedAchievementIds: ['achievement:first-steps', 'achievement:dockside-operator', 'achievement:first-rebirth'],skills: {}, empirePoints:4,rebirthCount:1});
    ...
    expect(second.permanentProgression).toEqual({ statistics: createInitialStatistics(2), unlockedAchievementIds: ['achievement:first-steps', 'achievement:dockside-operator', 'achievement:first-rebirth'],skills: {}, empirePoints:11,rebirthCount:2});
    expect(second.garage).toEqual(first.garage);
    expect(second).toEqual({...createInitialGameState(),garage:first.garage,permanentProgression:{ statistics: createInitialStatistics(2), unlockedAchievementIds: ['achievement:first-steps', 'achievement:dockside-operator', 'achievement:first-rebirth'],skills: {}, empirePoints:11,rebirthCount:2}});
  ...
  it('accepts exact maximum permanent sums',()=>{
    const state={...rebirthState(),permanentProgression:{ statistics: createInitialStatistics(0), unlockedAchievementIds: [],skills: {}, empirePoints:MAX_PERMANENT_VALUE-4,rebirthCount:MAX_PERMANENT_VALUE-1}};
    expect(performRebirth(state).state.permanentProgression).toEqual({ statistics: createInitialStatistics(1), unlockedAchievementIds: ['achievement:first-steps', 'achievement:dockside-operator', 'achievement:first-rebirth'],skills: {}, empirePoints:MAX_PERMANENT_VALUE,rebirthCount:MAX_PERMANENT_VALUE});
  });
  ...
  it('EP and Rebirth count provide no bonuses and never accrue offline', () => {
    const state=rebirthState();const permanent={...state,permanentProgression:{ statistics: createInitialStatistics(99), unlockedAchievementIds: [],skills: {}, empirePoints:999999,rebirthCount:99}};
    ...
    expect(offline.state.permanentProgression).toEqual({...unlockEligibleAchievements(permanent).state.permanentProgression,statistics:{...permanent.permanentProgression.statistics,automatedJobsCompleted:2880}});
