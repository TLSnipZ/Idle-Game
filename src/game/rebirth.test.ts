...
describe('Rebirth eligibility and rewards', () => {
  ...
  it.each([[20,25,4],[37,48,7],[100,100,20]])('preview and award agree: %i / %i => %i EP', (player,business,reward) => {
    ...
    expect(performRebirth(state)).toMatchObject({ok:true,reward,state:{permanentProgression:{empirePoints:reward,rebirthCount:1}}});
  });
...
describe('explicit reset and retention', () => {
  it('resets every temporary field, retains garage and accumulates permanent counters immutably', () => {
    const state={...rebirthState(),permanentProgression:{empirePoints:12,rebirthCount:3}};
    ...
    expect(after.permanentProgression).toEqual({empirePoints:16,rebirthCount:4});
    expect(after).toEqual({...createInitialGameState(),garage:state.garage,permanentProgression:{empirePoints:16,rebirthCount:4}});
  ...
  it('supports repeat Rebirths: 4 then 7 EP, count 2, same collection', () => {
    ...
    expect(first.permanentProgression).toEqual({empirePoints:4,rebirthCount:1});
    ...
    expect(second.permanentProgression).toEqual({empirePoints:11,rebirthCount:2});
    expect(second.garage).toEqual(first.garage);
    expect(second).toEqual({...createInitialGameState(),garage:first.garage,permanentProgression:{empirePoints:11,rebirthCount:2}});
  ...
  it.each(['empirePoints','rebirthCount'] as const)('rejects %s overflow without any reset', field => {
    const state={...rebirthState(),permanentProgression:{empirePoints:0,rebirthCount:0,[field]:MAX_PERMANENT_VALUE}};
  ...
  it('accepts exact maximum permanent sums',()=>{
    const state={...rebirthState(),permanentProgression:{empirePoints:MAX_PERMANENT_VALUE-4,rebirthCount:MAX_PERMANENT_VALUE-1}};
    expect(performRebirth(state).state.permanentProgression).toEqual({empirePoints:MAX_PERMANENT_VALUE,rebirthCount:MAX_PERMANENT_VALUE});
  });
  it('fails loudly on corrupt authoritative values without hiding them with a fresh run', () => {
    const state={...rebirthState(),permanentProgression:{empirePoints:-1,rebirthCount:0}};
  ...
  it('permanent counters provide no stats and never accrue offline', () => {
    const state=rebirthState();const permanent={...state,permanentProgression:{empirePoints:999999,rebirthCount:99}};
