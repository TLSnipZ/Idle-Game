...
describe('KX-R permanent tuning', () => {
  ...
  it('migrates v20 stock unchanged and round-trips tuned saves through CE1', () => {
    ...
    expect(old).toMatchObject({ ok: true, envelope: { version: 23, savedAt: 1234, state: s } });
    const tuned = buy(s), encoded = serializeSave(tuned, 1234); if (!encoded.ok) throw Error('fixture');
    expect(parseSave(encoded.serialized)).toMatchObject({ ok: true, envelope: { version: 23, state: tuned } });
