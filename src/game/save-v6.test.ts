...
describe('v6 vehicle saves',()=>{
  it('migrates realistic v5 preserving every previous field and savedAt through local/CE1 validation',()=>{
    ...
    const expected={ok:true,envelope:{...old,version:6,state:{...old.state,garage:{ownedVehicleIds:[]}}}};
    expect(CURRENT_SAVE_VERSION).toBe(6); expect(parseSave(serialized)).toEqual(expected);
  ...
  it('roundtrips exact v6 ownership and all progress through the same envelope',()=>{
    ...
    expect(parseSave(encoded.serialized)).toMatchObject({ok:true,envelope:{version:6,savedAt:42,state}});
  ...
  it('rejects missing garage, custom prototypes/accessors, future and malformed legacy envelopes',()=>{
    ...
    expect(parseSave(JSON.stringify({...legacy(),version:7}))).toEqual({ok:false,error:'unsupported-version'});
