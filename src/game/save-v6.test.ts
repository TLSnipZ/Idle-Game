...
function current() {
  ...
  return {...result.envelope.state,garage:{ ownedVehicleIds: [V.id], activeVehicleId: V.id }};
}
describe('v6 vehicle saves',()=>{
  ...
  it('roundtrips exact v6 ownership and all progress through the same envelope',()=>{
    ...
    expect(parseSave(encoded.serialized)).toMatchObject({ok:true,envelope:{version: 20,savedAt:42,state}});
  ...
  it('rejects missing garage, custom prototypes/accessors, future and malformed legacy envelopes',()=>{
    ...
    expect(validateSaveState({...current(),garage:Object.create({ ownedVehicleIds: [], activeVehicleId: null })})).toBeNull();
    ...
    expect(parseSave(stringifySaveFixture({...old,state:{...old.state,garage:{ ownedVehicleIds: [V.id], activeVehicleId: V.id }}})).ok).toBe(false);
  });
