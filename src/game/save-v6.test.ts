...
describe('v6 vehicle saves',()=>{
  it('migrates realistic v5 preserving every previous field and savedAt through local/CE1 validation',()=>{
    ...
    const expected={ok:true,envelope:{...old,version:9,state:{...old.state,city:{ownedTerritoryIds:['territory:waterfront']},permanentProgression:{skills: {}, empirePoints:0,rebirthCount:0},garage:{ownedVehicleIds:[]}}}};
    expect(CURRENT_SAVE_VERSION).toBe(9); expect(parseSave(serialized)).toEqual(expected);
  ...
  it('roundtrips exact v6 ownership and all progress through the same envelope',()=>{
    ...
    expect(parseSave(encoded.serialized)).toMatchObject({ok:true,envelope:{version:9,savedAt:42,state}});
