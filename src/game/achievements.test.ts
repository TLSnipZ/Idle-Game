...
describe('six permanent observational achievements', () => {
  ...
  it.each([0,1,2])('offline cap rank %i observes only the final Heat and preserves all credited subsystem results', rank => {
    const s = { ...milestone(3,59), automation: { businessAutoUpgradeTargetId: 'business:dockside-detail' as const, enabledIds: [], businessAutoUpgradeElapsedMs: 0, unlockedIds: [D.id], starterJobElapsedMs: 0 }, permanentProgression: { ...fresh().permanentProgression, skills: rank ? { 'skill:never-sleeps': rank } : {} } };
