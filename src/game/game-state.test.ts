...
describe('game state and starter delivery', () => {
  it('creates only the implemented authoritative slices', () => {
    ...
    expect(state).toEqual({ garage: { ownedVehicleIds: [] }, progression: { xp: 0 }, automation: { unlockedIds: [], starterJobElapsedMs: 0 }, upgrades: { purchasedIds: [] }, economy: { cash: '0' }, businesses: { productionRemainderSubMilliCents: { numerator: '0', denominator: '1' }, owned: {}, productionRemainderMilliCents: 0 } });
