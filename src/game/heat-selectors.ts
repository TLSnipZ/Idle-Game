...
export function selectHeat(state: GameState) {
  ...
  return { districtName: findTerritory(getActiveDistrictId(state.city))?.name ?? '', decayIntervalMs, heat, heatDecayElapsedMs, tier, maximum: MAX_HEAT, percentage: heat * 100 / MAX_HEAT,
