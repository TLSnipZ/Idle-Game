...
export function performStarterJob(state: GameState): StarterJobResult {
  return completeDelivery(state, evaluateJobReward(state), MANUAL_JOB_HEAT, true, false);
}
...
export function performRiskyDelivery(state: GameState): RiskyDeliveryResult {
  ...
  return completeDelivery(state, view.reward, view.heatGain, true, true);
}
...
export function performDiscreetDelivery(state: GameState): DiscreetDeliveryResult {
  ...
  const blocked = readinessFailure(state);
  if (blocked) return blocked;
  return completeDelivery(state, view.reward, -view.reduction, false, true);
}

function completeDelivery(state: GameState, reward: ReturnType<typeof evaluateJobReward>, heatGain: number, earnsXp = true, consumesSpecialReadiness = false): StarterJobResult {
  ...
  const candidate = { ...state, ...(consumesSpecialReadiness ? { manualJobs: consumeManualJobReadiness(state.manualJobs) } : {}), city: heatGain < 0 ? reduceHeat(state.city, -heatGain) : gainHeat(state.city, heatGain), progression: xp.state, economy: result.state };
