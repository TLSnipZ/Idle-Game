...
function completeDelivery(state: GameState, reward: ReturnType<typeof evaluateJobReward>, heatGain: number): StarterJobResult {
  ...
  const counted = countStatistic(state, { ...state, city: gainHeat(state.city, heatGain), progression: xp.state, economy: result.state }, 'manualJobsCompleted');
