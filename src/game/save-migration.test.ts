...
describe('v1 to v2 migration', () => {
  it('preserves exact cash, earned fraction and timestamp, assigns every owner level 1', () => {
    ...
    expect(result).toEqual({ ok: true, envelope: { ...old, version: CURRENT_SAVE_VERSION, state: {
      economy: old.state.economy, businesses: { owned: { [id]: { level: 1 } }, productionRemainderMilliCents: 975 },
    } } });
  ...
  it.each([0, -1, 1.5, 101, Number.MAX_SAFE_INTEGER + 1, '2', null])('rejects v2 level %#', level => {
    const old = legacy();
    expect(validateSaveState({ ...old.state, businesses: { owned: { [id]: { level } }, productionRemainderMilliCents: 975 } })).toBeNull();
  });
  ...
  it('does not reinterpret current payload as legacy or allow extra progression fields', () => {
    ...
    const old = legacy();
    expect(validateSaveState({ ...old.state, businesses: { owned: { [id]: { level: 1, rate: '75' } }, productionRemainderMilliCents: 975 } })).toBeNull();
  });
});
