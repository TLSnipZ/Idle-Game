...
describe('v1 migration through v2 ownership', () => {
  it('preserves exact cash, earned fraction and timestamp, assigns every owner level 1', () => {
    ...
    expect(result).toEqual({ ok: true, envelope: { ...old, version: CURRENT_SAVE_VERSION, state: {
      garage: { ownedVehicleIds: [] }, progression: { xp: 0 }, automation: { unlockedIds: [], starterJobElapsedMs: 0 }, upgrades: { purchasedIds: [] }, economy: old.state.economy, businesses: { productionRemainderSubMilliCents: { numerator: '0', denominator: '1' }, owned: { [id]: { level: 1 } }, productionRemainderMilliCents: 975 },
    } } });
