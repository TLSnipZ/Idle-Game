...
describe('permanent offline and Rebirth effects', () => {
  it.each([0, 1, 2])('Never Sleeps rank %i caps all subsystems together and preserves actual absence', rank => {
    ...
    for (const elapsed of [cap - 1, cap, cap + 4 * 3600000 + 9999]) {
      ...
      expect(result.state.permanentProgression).toEqual({...state.permanentProgression, statistics:{...state.permanentProgression.statistics,automatedJobsCompleted:Math.floor(Math.min(cap,elapsed)/10000)}, unlockedAchievementIds: ['achievement:first-steps','achievement:first-rebirth']});
    }
  });
