...
describe('v10 Heat migration and portable validation', () => {
  it('v9 migration only adds zero Heat/progress and preserves every existing value', () => {
    ...
    expect(stringifySaveFixture(envelope(legacyState,9))).toBe(raw); expect(validateSaveCode(encodeSaveText(raw))).toEqual(parsed);
  });
  it.each([1,2,3,4,5,6,7,8,9])('old CE1 v%i migrates through the sequential boundary with zero Heat', version => {
    ...
    expect(validateSaveCode(code)).toMatchObject({ ok: true, envelope: { version: 17, savedAt: 123456789,
  ...
  it.each([-1,101,.5,Number.MAX_SAFE_INTEGER+1,NaN,Infinity,'70',null,undefined])('rejects malformed Heat %s', heat => {
    ...
    expect(validateSaveState(state)).toBeNull();expect(parseSave(stringifySaveFixture(envelope(state)))).toEqual({ok:false,error:'invalid-state'});
  });
  it.each([-1,.5,60000,Number.MAX_SAFE_INTEGER+1,NaN,Infinity,'45000',null,undefined])('rejects malformed decay progress %s', heatDecayElapsedMs => {
    ...
    expect(validateSaveState(state)).toBeNull();expect(validateSaveCode(encodeSaveText(stringifySaveFixture(envelope(state))))).toEqual({ok:false,error:'invalid-state'});
  });
