...
function rich() {
  ...
  return { ...base, city: { ...territoryState(true).city, heat: 70, heatDecayElapsedMs: 45000 },
    permanentProgression: { empirePoints: 17, rebirthCount: 4,
...
describe('v10 Heat migration and portable validation', () => {
  it('v9 migration only adds zero Heat/progress and preserves every existing value', () => {
    const current = rich(); const old = { ...current, city: { ownedTerritoryIds: current.city.ownedTerritoryIds } };
    const raw = JSON.stringify(envelope(old,9)); const parsed = parseSave(raw);
    expect(CURRENT_SAVE_VERSION).toBe(10);
    expect(parsed).toEqual({ ok: true, envelope: envelope({ ...old, city: { ...old.city, heat: 0, heatDecayElapsedMs: 0 } }) });
    if (!parsed.ok) throw Error('fixture');
    const { city, ...previous } = parsed.envelope.state; const { city: oldCity, ...before } = old;
  ...
  it.each([1,2,3,4,5,6,7,8,9])('old CE1 v%i migrates through the sequential boundary with zero Heat', version => {
    ...
    expect(validateSaveCode(code)).toMatchObject({ ok: true, envelope: { version: 10, savedAt: 123456789,
  ...
  it('rejects banked cooling, accessors, missing fields and malformed city without repair', () => {
    ...
    expect(parseSave(JSON.stringify(envelope(s,11)))).toEqual({ok:false,error:'unsupported-version'});
