...
export type PurchaseAutomationResult = { readonly ok: true; readonly state: GameState }
  | { readonly ok: false; readonly state: GameState; readonly error: EconomyError | 'unknown-automation' | 'already-unlocked' | 'prerequisite-not-met' };
export function purchaseAutomation(state: GameState, id: unknown): PurchaseAutomationResult {
  ...
  if (!Object.hasOwn(state.businesses.owned, DELIVERY_DISPATCHER.requiredBusiness)) return { ok: false, state, error: 'prerequisite-not-met' };
