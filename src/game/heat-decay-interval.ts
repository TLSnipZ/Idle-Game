...
import { HEAT_DECAY_INTERVAL_MS, MIN_HEAT_DECAY_INTERVAL_MS } from '../features/heat';
...
export function getHeatDecayIntervalMs(state: GameState): number {
  ...
  return evaluateIntervalMs(interval, MIN_HEAT_DECAY_INTERVAL_MS, collectModifiers(state));
}
