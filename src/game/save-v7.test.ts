...
describe('v7 permanent progression schema',()=>{
  it('migrates realistic v6 preserving every prior field and timestamp; never performs Rebirth',()=>{
    const old=legacy();const text=JSON.stringify(old);expect(CURRENT_SAVE_VERSION).toBe(8);
    const expected={ok:true,envelope:{...old,version:8,state:{...old.state,permanentProgression:{skills: {}, empirePoints:0,rebirthCount:0}}}};
  ...
  it.each([1,2,3,4,5,6])('keeps sequential CE1 migration from v%i',version=>{
    ...
    expect(result).toMatchObject({ok:true,envelope:{version:8,savedAt:old.savedAt,state:{economy:source.economy,
      permanentProgression:{skills: {}, empirePoints:0,rebirthCount:0},businesses:{owned:{[B.id]:{level:version===1?1:48}},productionRemainderMilliCents:975}}}});
  });
  ...
  it('rejects missing slice, extra fields, wrong prototypes and future schemas',()=>{
    ...
    expect(parseSave(JSON.stringify({...legacy(),version:9}))).toEqual({ok:false,error:'unsupported-version'});
  ...
  it('retains the v6 timestamp for ordinary offline migration and consumes it once',()=>{
    ...
    expect(parseSave(raw)).toMatchObject({ok:true,envelope:{version:8,savedAt:wall,state:expected}});
