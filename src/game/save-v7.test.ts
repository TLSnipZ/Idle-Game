...
describe('v7 permanent progression schema',()=>{
  it('migrates realistic v6 preserving every prior field and timestamp; never performs Rebirth',()=>{
    const old=legacy();const text=stringifySaveFixture(old);expect(CURRENT_SAVE_VERSION).toBe(21);
  ...
  it.each([1,2,3,4,5,6])('keeps sequential CE1 migration from v%i',version=>{
    ...
    expect(result).toMatchObject({ok:true,envelope:{version: 21,savedAt:old.savedAt,state:{economy:source.economy,
  ...
  it.each([0,11,MAX_PERMANENT_VALUE])('roundtrips exact counters %i without runtime/UI state',value=>{
    ...
    expect(parseSave(serialized.serialized)).toMatchObject({ok:true,envelope:{version: 21,savedAt:42,state}});
  ...
  it('retains the v6 timestamp for ordinary offline migration and consumes it once',()=>{
    ...
    expect(parseSave(raw)).toMatchObject({ok:true,envelope:{version: 21,savedAt:wall,state:expected}});
