...
const envelope = <T>(state: T, version = 21) => ({ format: 'crime-empire-save', version, savedAt: 123456789, state });
...
describe('v17 target compatibility', () => {
  it('adds only the stable target to fresh automation; new Businesses remain absent', () => {
    const s = createInitialGameState(); expect(CURRENT_SAVE_VERSION).toBe(21);
