  ...
  return <section className={`next-objective ${expanded ? 'is-expanded' : 'is-collapsed'}`} aria-labelledby="next-objective-heading">
    <div className="objective-compact">
      <div className="objective-compact-copy">
        ...
        <p className="objective-compact-progress"><strong>{compactProgress}</strong><span> · {text('Solara recommends questionable ambition.', 'Solara empfiehlt fragwürdigen Ehrgeiz.')}</span></p>
      </div>
      <button type="button" className="objective-expand" aria-expanded={expanded} aria-controls="next-objective-details" onClick={() => setExpanded(value => !value)}>
        {expanded ? text('Less', 'Weniger') : text('Details', 'Details')} <span aria-hidden="true">{expanded ? '▴' : '▾'}</span>
      </button>
    </div>
    {expanded && <div id="next-objective-details" className="objective-details">
      ...
      <div className="objective-controls">
        <button type="button" className="action-button secondary-button" aria-label={text(`${view.button} for your next objective`, `${view.button} für dein nächstes Ziel`)} onClick={() => onNavigate(step.destination)}>{view.button}</button>
        <details>
