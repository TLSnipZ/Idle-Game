...
export function ModifierBreakdown({ modifiers }: { readonly modifiers: readonly Modifier[] }) {
  return <ul className="modifier-breakdown">{modifiers.map(modifier =>
    <li key={modifier.id}>{findUpgrade(modifier.sourceId)?.name ?? findVehicle(modifier.sourceId)?.name ?? findSkill(modifier.sourceId)?.name ?? modifier.sourceId}: {formatModifier(modifier)}</li>)}</ul>;
}
