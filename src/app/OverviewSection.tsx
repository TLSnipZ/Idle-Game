...
export function OverviewSection({ state, paused, onNavigate }: {
  ...
  return <div className="overview-command">
    ...
    <article className="panel summary-card overview-empire"><h2>{text('EMPIRE', 'IMPERIUM')}</h2><p className="summary-value">{text(formatInteger(view.empire.empirePoints))} {text('EP')}</p>
      <p>{text('Rebirths:', 'Rebirths:')} {text(formatInteger(view.empire.rebirthCount))}</p><p>{view.empire.eligible ? text('Rebirth requirements met — the reset button is wearing a suit now.', 'Rebirth-Voraussetzungen erfüllt — der Reset-Knopf trägt jetzt Anzug.') : text('Build toward your next Rebirth. Temporary money, permanent ego.', 'Arbeite auf den nächsten Rebirth hin. Temporäres Geld, permanentes Ego.')}</p>
