...
export function OverviewSection({ state, paused, onNavigate }: {
  ...
  return <div className="overview-command">
    ...
    <article className={`panel summary-card overview-pressure heat-${view.heat.tier.id}`}><h2>{text('CITY PRESSURE', 'DRUCK VON OBEN')}</h2><p className="summary-value">{text('Heat')} {view.heat.heat} / {view.heat.maximum} · {heat.tier.label}</p>
    ...
    <article className="panel summary-card overview-empire"><h2>{text('EMPIRE', 'IMPERIUM')}</h2><p className="summary-value">{formatInteger(view.empire.empirePoints)} {text('EP')}</p>
    ...
    <article className={`panel summary-card overview-event ${view.event.pending ? 'is-pending' : 'is-idle'}`}><h2>{text('CITY EVENT', 'STADTEVENT')}</h2><p>{eventName ?? text('No active event', 'Kein aktives Event')}</p>
      ...
      {view.event.pending && <button className="action-button section-shortcut" onClick={() => onNavigate(SECTION.city.id, 'city-events-heading')}>{text('VIEW EVENT', 'EVENT ANSEHEN')}</button>}</article>
    <article className="panel summary-card overview-crew"><h2>{text('CREW', 'CREW')}</h2><p>{text('Recruited:', 'Rekrutiert:')} {view.crew.recruitedCrewCount} / {view.crew.totalConfiguredCrew}</p>
      <p>{text('Active assignments:', 'Aktive Einsätze:')} {view.crew.activeAssignmentCount} / {view.crew.totalSlots}</p><p>{text('Only assigned specialists provide bonuses. Standing around remains unpaid.', 'Nur zugewiesene Spezialisten liefern Boni. Rumstehen bleibt unbezahlt.')}</p><button className="action-button section-shortcut" onClick={() => onNavigate(SECTION.city.id, 'crew-heading')}>{text('VIEW CREW', 'CREW ÖFFNEN')}</button></article>
