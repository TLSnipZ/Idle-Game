...
export function VehicleTuning({ state, paused, onConfigure, selectedVehicle, onChooseVehicle }: {
  ...
  return <section className="vehicle-tuning panel" aria-labelledby="tuning-heading">
    ...
    <div className="workshop-stage"><VehicleArtwork vehicleId={vehicle.id} appearanceId={state.garage.appearances?.[vehicle.id] ?? null} /><div><p className="eyebrow">{text('ON THE LIFT', 'AUF DER HEBEBÜHNE')}</p><h4>{text(vehicle.name)}</h4><p>{text('Current setup', 'Aktuelles Setup')}: <strong>{selected ? text(selected.name, selected.germanName) : text('Stock', 'Serie')}</strong></p><p>{active ? text('Active vehicle · bonuses enabled', 'Aktives Fahrzeug · Boni wirksam') : text('Workshop selection · vehicle remains inactive', 'Werkstatt-Auswahl · Fahrzeug bleibt inaktiv')}</p></div></div>
    ...
    <details className="workshop-help"><summary>{text('How setups work', 'So funktionieren Setups')}</summary>    <p>{text('One setup per car. Buy permanent parts, switch owned setups or restore stock for free. Rebirth keeps every build. Your accountant keeps every nightmare.',
      'Ein Setup pro Auto. Teile dauerhaft kaufen, gekaufte Setups oder Serie kostenlos wechseln. Rebirth behält jeden Ausbau. Dein Buchhalter behält jeden Albtraum.')}</p>    <p>{text('The base vehicle bonus remains. Only one fitted setup adds its effect while that car is active. Choosing a workshop or fitting parts does not activate the car.',
      'Der Basisbonus bleibt. Nur ein eingebautes Setup wirkt zusätzlich, solange dieses Auto aktiv ist. Werkstatt-Auswahl und Einbau aktivieren das Auto nicht.')}</p></details>
