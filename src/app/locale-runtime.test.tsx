...
it('keeps each mounted runtime locale across command renders and separates instances', async () => {
  const container = document.createElement('div');
  document.body.append(container);
  const root = createRoot(container);
  const games = new Map<number, ReturnType<typeof useGame>>();
  function Harness({ id }: { readonly id: number }) {
    const game = useGame(); games.set(id, game); return null;
  }
