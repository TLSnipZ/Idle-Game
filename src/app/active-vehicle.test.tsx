...
it.each(['en', 'de'] as const)('%s exposes exact active/inactive state and dispatches only the chosen vehicle', async (locale: Locale) => {
  ...
  expect(container.querySelector('button')?.getAttribute('aria-label')).toBe(locale === 'en' ? 'Activate Kairo KX-R' : 'Kairo KX-R aktivieren');
  await render(true);
  const paused = container.querySelector<HTMLButtonElement>('button');
