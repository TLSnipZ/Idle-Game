...
export function AutoUpgraderCard({ view, paused, onPurchase, onToggle, onTargetChange }: {
  ...
  return <section className="panel upgrade-panel automation-card auto-spend-card" aria-labelledby="auto-upgrader-heading">
    ...
    <p id="auto-upgrader-description">{description}</p>
    <div className="automation-metrics">
      <div><span>{text('Attempt', 'Versuch')}</span><strong>{text(formatRemainingTime(view.definition.intervalMs))}</strong></div>
      <div><span>{text('Target', 'Ziel')}</span><strong>{view.owned ? text(view.target.name) : '—'}</strong></div>
    </div>
    {view.owned ? <>
      {view.targets.length > 1 || (view.level === null && view.targets.length > 0) ? <div className="auto-target">
        ...
        <select id="auto-upgrader-target" value={view.level === null ? '' : view.target.id} disabled={paused} onChange={event => onTargetChange?.(event.currentTarget.value)}>
          ...
          {view.targets.map(target => <option key={target.id} value={target.id}>{text(target.name)}</option>)}
        </select>
      </div> : null}
      <div className="automation-next-state">
        <span>{view.level === null ? text('No owned target', 'Kein eigenes Ziel') : text(`${view.target.name} · Level ${view.level}`)}</span>
        <strong>{view.maxed ? text('MAXED', 'MAX') : view.nextCost !== null ? formatPrice(view.nextCost) : '—'}</strong>
      </div>
      <label id="auto-upgrader-timing" htmlFor="auto-upgrader-progress">{text('Next attempt in', 'Nächster Versuch in')} <strong>{text(formatRemainingTime(view.remainingMs))}</strong>{!view.enabled || paused ? text(' · Progress paused', ' · Fortschritt pausiert') : ''}</label>
      ...
      <details className="operations-disclosure"><summary>{text('Spending details', 'Ausgaben-Details')}</summary><div className="operations-disclosure-body"><p id="auto-upgrader-spending">{text('Automatically buys one upgrade for the selected Business every 30s when affordable.', 'Kauft alle 30s automatisch ein Upgrade fürs gewählte Business, wenn genug Cash da ist.')}</p>{!view.maxed && !view.canAffordNextUpgrade && <p>{text('More cash needed for the next upgrade.', 'Für das nächste Upgrade fehlt noch Cash.')}</p>}</div></details>
      <button className="action-button secondary-button" aria-describedby="auto-upgrader-spending" disabled={paused || !view.canToggle} aria-label={text(`${view.enabled ? 'Disable' : 'Enable'} Business Auto-Upgrader`, `Business Auto-Upgrader ${view.enabled ? 'deaktivieren' : 'aktivieren'}`)} onClick={() => onToggle(!view.enabled)}>{view.enabled ? text('DISABLE', 'DEAKTIVIEREN') : text('ENABLE', 'AKTIVIEREN')}</button>
    </> : <>
      <p id="auto-upgrader-consent">{text('Starts disabled. Enable it to automatically spend Cash on upgrades. Your accountant would like a word.', 'Startet deaktiviert. Erst beim Aktivieren gibt er automatisch Cash für Upgrades aus. Die Buchhaltung bittet um ein Gespräch.')}</p>
      <div className="automation-price"><span>{text('Price', 'Preis')}</span><strong>{formatPrice(view.definition.purchaseCost)}</strong></div>
      {view.requirements.requirements.length > 0 && <details className="operations-disclosure compact-requirements"><summary>{text('Requirements', 'Voraussetzungen')}</summary><RequirementList result={view.requirements} id="auto-upgrader-requirements" /></details>}
      <div className="card-action-area"><button className="action-button purchase-button" aria-describedby="auto-upgrader-description auto-upgrader-consent auto-upgrader-requirements" disabled={paused || !view.canPurchase} onClick={onPurchase}>{text('Buy Business Auto-Upgrader', 'Business Auto-Upgrader kaufen')}</button>{acquisition.note && <p className="purchase-note">{acquisition.note}</p>}</div>
    </>}
  </section>;
}
