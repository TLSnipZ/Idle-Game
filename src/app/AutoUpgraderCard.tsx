...
export function AutoUpgraderCard({ view, paused, onPurchase, onToggle, onTargetChange }: {
  ...
  return <section className="panel upgrade-panel automation-card auto-spend-card" aria-labelledby="auto-upgrader-heading">
    ...
    {view.owned ? <>
      ...
      <div className="card-action-area"><button className="action-button purchase-button" aria-describedby="auto-upgrader-description auto-upgrader-requirements" disabled={paused || !view.canPurchase} onClick={onPurchase}>{text('Buy Business Auto-Upgrader', 'Business Auto-Upgrader kaufen')}</button>{acquisition.note && <p className="purchase-note">{acquisition.note}</p>}</div>
    </>}
  </section>;
}
