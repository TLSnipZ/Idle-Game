...
export function SkillTree({ state, paused, onPurchase }: {
  ...
  return <section className="skill-tree" aria-labelledby="skill-tree-heading">
    <div className="panel-heading"><h2 id="skill-tree-heading">{EMPIRE_FOUNDATIONS.name}</h2>
      <span>Empire Points available: {formatInteger(state.permanentProgression.empirePoints)} EP</span></div>
    ...
    <div className="skill-nodes">{SKILL_CATALOG.map(skill => {
      ...
      return <article key={skill.id} className={`panel skill-node ${skill.requirements.length === 0 ? 'skill-foundation' : 'skill-branch'}`} aria-labelledby={heading}>
        <div className="panel-heading"><h3 id={heading}>{skill.name}</h3>
          <span className="ownership-badge">{view.maxed ? 'MAXED · MAX RANK' : !view.requirements.met ? 'LOCKED' : view.insufficientEp ? 'AVAILABLE · INSUFFICIENT EP' : 'READY TO PURCHASE'}</span></div>
        ...
        {!view.maxed && <>
          <p>Next rank: <strong>{view.nextCost === null ? '—' : formatInteger(view.nextCost)} EP</strong></p>
