...
export function GlobalFeedback({ game, transferMessage, rebirthMessage }: {
  ...
  return <aside className="global-feedback legacy-feed-announcer" aria-label={text('Session feedback', 'Session-Rückmeldungen')}>
    <div className="legacy-feed-announcements">
      <div className={`feedback-command feedback-${feedback.tone ?? 'info'}`} role="status" aria-live="polite" aria-atomic="true"><span key={feedback.sequence}>{text(feedback.message)}</span></div>
      ...
      <div role="status" aria-live="polite" aria-atomic="true">{levelEvent && !paused && <span key={levelEvent.sequence}>{describeLevelIncrease(levelEvent, locale)}{levelEvent.unlocks?.length ? ` · ${'New unlock available:', 'Neue Freischaltung:'} ${levelEvent.unlocks.map(name => localizedUnlock(name, locale)).join(', ')}` : ''}</span>}</div>
      ...
      {transferMessage && <p role="status">{text(transferMessage)}</p>}
      {rebirthMessage && <p role="status">{text(rebirthMessage)}</p>}
    </div>
