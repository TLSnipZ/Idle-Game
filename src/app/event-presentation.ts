...
export function describeChoiceEffects(choice: EventChoice): readonly string[] {
  const lines: string[] = [];
  if (choice.cost !== '0') lines.push(`${choice.reward !== '0' ? 'Cost: ' : '-'}${formatReward(choice.cost)}`);
  if (choice.reward !== '0') lines.push(`${choice.cost !== '0' ? 'Return: ' : '+'}${formatReward(choice.reward)}`);
...
export function eventPresentation(state: GameState) {
  ...
  return {...view,countdown:seconds === null ? null : `${Math.floor(seconds/60).toString().padStart(2,'0')}:${(seconds%60).toString().padStart(2,'0')}`,
    choices:view.choices.map(view=>({...view,effects:describeChoiceEffects(view.choice),unavailable:view.affordable ? null : `Requires ${formatReward(view.choice.cost)} to choose this option`}))};
}
export function describeEventResolution(result: EventResolutionResult): string {
  ...
  return `${choice.outcome} — ${describeChoiceEffects(choice).join(' · ')}${net?.ok ? ` · net +${formatReward(net.value)}` : ''}.`;
}
