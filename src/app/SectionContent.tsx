...
export function SectionContent({ active, game, onNavigate, save, rebirth }: SectionContentProps) {
  ...
  switch (active) {
    ...
    case SECTION.collection.id: return <><Garage state={state} paused={paused || game.persistence.kind === 'blocked'} onPurchase={game.buyVehicle} onSelect={game.chooseActiveVehicle} /><VehicleTuning state={state} paused={paused || game.persistence.kind === 'blocked'} onConfigure={game.configureTuning} /></>;
