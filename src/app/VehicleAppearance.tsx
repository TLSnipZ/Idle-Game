...
export function VehicleAppearance({ state, paused, onApply }: {
  ...
  const finishes = APPEARANCE_CATALOG.filter(look => look.vehicleId === vehicle.id);
  ...
  return <section className="vehicle-appearance panel" aria-labelledby="appearance-heading">
    ...
    <p>{text('Signature finishes for supported models. Same questionable business model, better curb appeal.',
      'Besondere Lackierungen für unterstützte Modelle. Dasselbe fragwürdige Geschäftsmodell, besserer erster Eindruck.')}</p>
    ...
    <div className="paint-studio-layout">
      ...
      <div className="paint-controls">
        {finishes.length === 0 ? <p className="stock-only-notice">{text('Factory finish only. More paint choices for this model are not available yet.',
          'Nur Werkslackierung. Weitere Lackierungen sind für dieses Modell noch nicht verfügbar.')}</p> : <>
