...
export function AutomationCard({ view, paused, onPurchase, event }: {
  ...
  return <section className="panel upgrade-panel automation-card dispatcher-card" aria-labelledby="delegation-heading">
    ...
    <div className="automation-metrics">
      <div><span>{text('Interval', 'Intervall')}</span><strong>{text(formatRemainingTime(view.intervalMs))}</strong></div>
      <div><span>{text('Payout', 'Auszahlung')}</span><strong>{view.reward === null ? '—' : formatCash(view.reward)}</strong></div>
    </div>
    {view.unlocked ? <>
      <label id="dispatcher-timing" htmlFor="dispatcher-progress">{text('Next delivery in', 'Nächste Lieferung in')} <strong>{text(formatRemainingTime(view.remainingMs))}</strong>{paused ? text(' · Session paused', ' · Session pausiert') : ''}</label>
      ...
      <details className="operations-disclosure"><summary>{text('Dispatcher details', 'Dispatcher-Details')}</summary><div className="operations-disclosure-body"><p>{text('XP')}: <strong>{XP_REWARDS.dispatcherJob}</strong> {text('base per delivery.', 'Basis pro Lieferung.')}</p><p>{text('Heat')}: <strong>+1</strong> {text(`per ${DISPATCHER_JOBS_PER_HEAT} deliveries completed together.`, `pro ${DISPATCHER_JOBS_PER_HEAT} gemeinsam abgeschlossene Lieferungen.`)}</p><p>{text('Manual deliveries remain available and do not reset this progress.', 'Manuelle Lieferungen bleiben möglich und setzen den Timer nicht zurück.')}</p><p>{event ? text(`Last dispatch: ${describeAutomatedJobs(event, locale)}`, `Letzter Dispatch: ${describeAutomatedJobs(event, locale)}`) : text('Your dispatcher is ready for the next run.', 'Dein Dispatcher ist bereit. Motivation wurde nicht geprüft.')}</p></div></details>
    </> : <>
      ...
      <div className="card-action-area"><button className="action-button purchase-button" aria-describedby="dispatcher-requirement" disabled={paused || !view.canPurchase}
