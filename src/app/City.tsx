...
export function City({ state, paused, onAcquire, onLayLow, onChooseDistrict }: {
  readonly onChooseDistrict?: (id: string) => void;
  ...
  return <section className="city" aria-labelledby="city-heading">
    ...
    {onChooseDistrict && <DistrictHeat nested state={state} paused={paused} onChoose={onChooseDistrict} />}
    ...
    <div className="territory-catalog">{TERRITORY_CATALOG.map(territory => {
      ...
      return <article key={territory.id} className={`panel territory-card ${view.owned ? 'is-owned' : ''}`} aria-labelledby={heading}>
        ...
        {view.owned ? <p>{view.availability}</p> : <>
          <p className="acquisition-warning">{text(`Acquisition generates +${territory.acquisitionHeat} Heat. Waterfront paperwork; Neon Mile starts cold.`, `Übernahme erzeugt +${territory.acquisitionHeat} Heat. Papierkram in Waterfront; Neon Mile startet kalt.`)}</p>
