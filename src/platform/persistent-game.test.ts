...
describe('level command persistence and time boundaries', () => {
  ...
  it('migrates v1 local saves before offline simulation and consumes original timestamp once', () => {
    ...
    expect(game.getSnapshot().result.state).toEqual(simulateElapsed({ ...owned(), progression: { xp: 0 } }, 1000).state);
    ...
    expect(second.getSnapshot().result.state).toEqual({ ...owned(), progression: { xp: 0 } });
  });
});
