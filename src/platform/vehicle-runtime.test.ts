...
function initial(owned=false): GameState {
  ...
  return {...state,garage:{ ownedVehicleIds: owned ? [V.id] : [], activeVehicleId: owned ? V.id : null },progression:{xp:3600},
...
describe('vehicle runtime and durable progression',()=>{
  ...
  it('exports current state, autosaves conservatively, reloads and remounts without duplicate loops',()=>{
    ...
    expect(validateSaveCode(exported.code)).toMatchObject({ok:true,envelope:{version: 19,savedAt:6000,state:expected}});
...
it('v5 local migration consumes its saved timestamp without losing offline time',()=>{
  ...
  expect(parseSave(raw)).toMatchObject({ok:true,envelope:{version: 19,savedAt:26000,state:expected}});
...
it.each([false, true])('historical v15 CE1 owner=%s imports atomically without historical income and re-exports v17', owner => {
  ...
  expect(validateSaveCode(exported.code)).toMatchObject({ ok: true, envelope: { version: 19, savedAt: 1000000, state } });
...
it('historical owner bootstrap migrates before offline evaluation and persists current +10% once', () => {
  ...
  const state = { ...fresh, businesses: { ...fresh.businesses, owned: { [B.id]: { level: 4 } } }, garage: { ownedVehicleIds: [V.id], activeVehicleId: V.id } };
