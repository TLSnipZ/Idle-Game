...
function initial(owned=false): GameState {
  ...
  return {...state,garage:{ownedVehicleIds:owned?[V.id]:[]},progression:{xp:3600},
    ...
    upgrades:{purchasedIds:UPGRADE_CATALOG.map(u=>u.id)},automation:{unlockedIds:[D.id],starterJobElapsedMs:5000}};
}
...
describe('vehicle runtime and durable progression',()=>{
  ...
  it('exports current state, autosaves conservatively, reloads and remounts without duplicate loops',()=>{
    ...
    expect(validateSaveCode(exported.code)).toMatchObject({ok:true,envelope:{version:10,savedAt:6000,state:expected}});
...
it('v5 local migration consumes its saved timestamp without losing offline time',()=>{
  const state=initial();const {city:_city,permanentProgression:_permanent,garage:_garage,...legacy}=state;
  ...
  expect(parseSave(raw)).toMatchObject({ok:true,envelope:{version:10,savedAt:26000,state:expected}});
