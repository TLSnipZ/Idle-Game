...
describe('shared Heat runtime and durable boundaries', () => {
  ...
  it('already-cold after reconciliation fails without spending; elapsed results remain authoritative', () => {
    const s={...initial(1),automation: { businessAutoUpgradeTargetId: 'business:dockside-detail' as const,enabledIds:[],businessAutoUpgradeElapsedMs:0,unlockedIds:[],starterJobElapsedMs:0}},f=rebirthRuntime(s),raw=f.raw();
  ...
  it('autosave/export retain exact Heat progress; immediate reload and Strict Mode do not double-cool', () => {
    ...
    expect(validateSaveCode(code.code)).toMatchObject({ok:true,envelope:{version: 17,savedAt:6000,state:saved}});
