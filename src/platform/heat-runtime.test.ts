...
describe('shared Heat runtime and durable boundaries', () => {
  it('Neon acquisition reconciles old tier/ownership, then spends and adds Heat before future rewards', () => {
    ...
    const r=simulateGameElapsed(s,50000);expect(r.state.city).toMatchObject({heat:51,heatDecayElapsedMs:50000});
  ...
  it('Lay Low reconciles old HOT jobs and cooling first; future jobs use reduced Heat', () => {
    ...
    const elapsed=simulateGameElapsed(s,50000);const expected=layLow(elapsed.state).state;
  ...
  it('already-cold after reconciliation fails without spending; elapsed results remain authoritative', () => {
    ...
    expect(f.game.getSnapshot().result.state).toEqual(simulateGameElapsed(s,60000).state);expect(f.raw()).toBe(raw);f.game.stop();
  });
  ...
  it('autosave/export retain exact Heat progress; immediate reload and Strict Mode do not double-cool', () => {
    ...
    expect(validateSaveCode(code.code)).toMatchObject({ok:true,envelope:{version:10,savedAt:6000,state:saved}});
  ...
  it('failed import preserves live/durable Heat and timing', () => {
    ...
    expect(f.game.getSnapshot().result.state).toEqual(simulateGameElapsed(state,10000).state);f.game.stop();
  });
  it.each([false,true])('Rebirth reconciles then resets Heat only after a successful durable write (failure=%s)', failed => {
    ...
    if(failed){expect(f.game.getSnapshot().result.state).toEqual(simulateGameElapsed(state,50000).state);expect(f.raw()).toBe(raw);}
    else {
  ...
  it.each([0,1,2])('offline at derived cap rank %i is durable, shared, one-time and ignores discarded time', rank => {
    ...
    const make=()=>createPersistentGame(v=>{if(v.persistence.kind==='loaded')events.push('publish');},createLocalSave(()=>({getItem:()=>raw,setItem:(_key:string,v:string)=>{raw=v;events.push('write');}}),()=>now),{now:()=>0,schedule:()=>()=>{}},()=>()=>{});
  ...
  it('failed offline write pauses startup and preserves original Heat/save without timers', () => {
    ...
    const game=createPersistentGame(()=>{},createLocalSave(()=>({getItem:()=>encoded.serialized,setItem:()=>{throw Error('quota');}}),()=>600000),{now:()=>0,schedule:()=>{timers++;return()=>{};}},()=>{timers++;return()=>{};});
  ...
  it('future timestamp rebases with no Heat gain/cooling, and runtime corruption suspends atomically', () => {
    ...
    const runtime=createGameRuntime(bad,()=>{},{now:()=>now,schedule:()=>()=>{cancelled=true;}});runtime.start();now=50000;
