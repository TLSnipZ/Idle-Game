...
import { KAIRO_SENDA as S, NAMERA_LILT as L, NAMERA_SEREIN as N, TOSEKI_RENDAN as R, SEVRIN_CANTO_CLUB as C, STARTER_VEHICLE as K, VEHICLE_CATALOG } from '../features/vehicles';
...
it.each([S.id, L.id, N.id, R.id, C.id])('real %s reconciles old effects, saves before publication and applies future effects', id => {
...
it.each([S.id, L.id, N.id, R.id, C.id])('%s saved effects govern offline credit once, reload and CE1', id => {
...
it.each([S.id, L.id, N.id, R.id, C.id])('%s survives Rebirth below purchase gates; New Game clears all vehicles', id => {
...
it('imports the complete Garage without historical earnings and retains selection after reload', () => {
