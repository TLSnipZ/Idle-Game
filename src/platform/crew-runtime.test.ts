...
describe('Crew runtime boundaries and persistence',()=>{
  it('recruitment reconciles first, preserves assignment and saves only the complete purchase',()=>{
    ...
    expect(parseSave(f.raw())).toMatchObject({ok:true,envelope:{version: 25,savedAt:13000,state:unlockEligibleAchievements(expected).state}});
  });
  ...
  it('saves recruit/assign/unassign and preserves explicit state through autosave, export, reload and Strict Mode',()=>{
    ...
    const code=f.game.exportCode();if(!code.ok)throw Error('fixture');expect(validateSaveCode(code.code)).toMatchObject({ok:true,envelope:{version: 25,savedAt:5321,state:expected}});
